...
clear;clear;clear
 ...
 # // Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "  Selamat Datang di SCRIPT TUNNELING ${YELLOW}(${NC}${green} Edisi Stabil ${NC}${YELLOW})${NC}"
echo -e "  Ini Akan Membantu Anda Mengatur Server VPN Dengan Cepat"
echo -e "  Dikembangkan oleh: ${green} SCRIPT TUNELLING ® ${NC}${YELLOW}(${NC} ${green}  TUNELLING ${NC}${YELLOW})${NC}"
echo -e "  Direkode Ulang oleh SCRIPT TUNELLING ${YELLOW}(${NC} 2022 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 2
###### IZIN SC
...
# USERNAME
rm -f /usr/bin/user
username=$(curl ${repo}-Run/main/user/user | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl ${repo}-Run/main/user/user | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl ${repo}-Run/main/user/user | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# REPO    
    instalasi="https://raw.githubusercontent.com/proj3c/-Run/main/"
...
function pasang_domain() {
    ...
    clear
    echo -e "   .----------------------------------."
    echo -e "   |\e[1;32mSilakan Pilih Tipe Domain Di Bawah Ini \e[0m|"
    echo -e "    '----------------------------------'"
    echo -e "     1. Domain Sendiri"
    echo -e "     2. Gunakan Domain Random Khusus DigitalOcean atau ISP LAIN"
    echo -e "    '------------------------------------"
    read -p "   Silakan pilih nomor 1-2 atau Tekan Tombol Apapun (Random) : " host
    echo ""
    
    if [[ $host == "1" ]]; then
        echo -e "   \e[1;32mSilakan Masukkan Subdomain Anda \e[0m"
        ...
        echo -e "   Penggunaan Subdomain/Domain Random"
        clear
    fi
}
...
<code>────────────────────</code>
<i>Notifikasi Otomatis Dari Instalasi Github</i>
"'&reply_markup={"inline_keyboard":[[{"text":"Daftarkan IP","url":"https://wa.me/6282217067357"}]]}' 
